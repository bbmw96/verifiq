...
public sealed class SqliteRulesDatabase : IRulesDatabase, IDisposable
{
        ...
        string ifcClass, string classificationCode, CountryMode mode)
    {
        ...
        // ── SINGAPORE IFC+SG — classification-driven SGPset_ requirements ─────
        if (mode == CountryMode.Singapore || mode == CountryMode.Combined)
        {
            // External elements → thermal requirements mandatory
            if (code.Contains("EXT") || code.Contains("EXTERNAL") || code.Contains("-EW") || code.Contains("-RF"))
            {
            ...
            if (code.Contains("STR") || code.Contains("STRUCTURAL") || code.Contains("LOAD") || code.Contains("-SB") || code.Contains("-LB"))
            {
                if (cls is "IFCWALL" or "IFCWALLSTANDARDCASE")
            ...
            if (code.Contains("FIR") || code.Contains("FIRE") || code.Contains("FRR") || code.Contains("-FR"))
            {
            ...
            if (code.Contains("ACC") || code.Contains("ACCESSIBLE") || code.Contains("DISABLE") || code.Contains("-AC"))
            {
                if (cls == "IFCDOOR")
                {
